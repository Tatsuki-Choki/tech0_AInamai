...
async def seed_books(session):
    ...
    existing_books_list = existing_books.scalars().all()
    
    count = 0
    for data in BOOKS_DATA:
        # Check if book exists
        book = next((b for b in existing_books_list if b.title == data["title"]), None)
        ...
        if book:
            ...
            book.recommended_comment = data["recommended_comment"]
    ...
    await session.commit()
    print(f"Processed books. Created {count} new books.")
