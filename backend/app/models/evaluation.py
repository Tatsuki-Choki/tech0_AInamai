...
class Evaluation(BaseModel):
    ...
    self_score = Column(JSONB, nullable=True)  # 自己評価（能力別カウント）
    ai_score = Column(JSONB, nullable=True)  # AI調整評価
    ai_comment = Column(Text, nullable=True)  # AIからの評価コメント
    teacher_score = Column(JSONB, nullable=True)  # 教師訂正評価
    teacher_comment = Column(Text, nullable=True)  # 教師からのフィードバック
    final_score = Column(JSONB, nullable=True)  # 最終評価

    # 評価した教師
    evaluated_by = Column(UUID(as_uuid=True), ForeignKey("teachers.id"), nullable=True)
