...
# 以下の値は環境変数または実際のシークレットに置き換えてください
...
# スタートアップコマンド
az webapp config set --resource-group "$RESOURCE_GROUP" --name "$APP_NAME" --startup-file "ls -R /home/site/wwwroot; pip freeze; env PYTHONPATH=\$PYTHONPATH:/home/site/wwwroot python -m gunicorn --bind=0.0.0.0:8000 --workers=1 --timeout=1800 --access-logfile - --error-logfile - -k uvicorn.workers.UvicornWorker app.main:app"
